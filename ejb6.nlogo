...
threads-own [ idle? server-id ttl ]

breed [ beans bean ]
beans-own [ idle? server-id ttl ]
...
to go
   ...
   repeat tx-volume [
      ...
      ; Activate thread.
      ask one-of threads with [server-id = current-server and idle?] [ 
         activate-thread tx-duration
      ]
   ...
   ask beans [
      set ttl ttl - 1
      if ttl = 0 [ ifelse idle? [ die ] [ deactivate-bean ] ]
   ]

   ; Clean up threads.
   ask threads with [not idle?] [
      set ttl ttl - 1
      if ttl = 0 [ deactivate-thread ]
   ]
...
to overflow-threads
   repeat threads-increase [ create-thread ]
   ask server current-server [
      set overflow-count overflow-count + threads-increase
      set color orange
   ]
end
...
to layout-threads
   ...
   __layout-circle sort-by [color-of ?1 < color-of ?2] threads max-pxcor
end
...
to layout-beans
   ...
   __layout-circle sort-by [color-of ?1 < color-of ?2] beans max-pxcor - 1
end
