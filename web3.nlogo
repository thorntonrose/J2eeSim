...
sessions-own [ ttl server-id thread-count ]
...
to setup
   ...
   repeat count servers [
      ;repeat tx-volume / (count servers) [ create-session ]
      repeat 180 [ create-session ]
      set current-server current-server + 1
   ]
...
to go
   set tx-per-user round(tx-volume / (max-users / tx-volume))
   ...
   repeat tx-volume [
      ...
      ; Check for session.
      if count sessions with [server-id = current-server and thread-count < tx-per-user] = 0 [
         create-session
      ]

      ; Add tx to session.
      let s one-of sessions with [server-id = current-server and thread-count < tx-per-user]
      ask s [ add-tx-to-session ]
   ...
   ask threads with [not idle?] [
      ...
      if ttl = 0 [ 
         ask tx-session [ remove-tx-from-session ]
         deactivate-thread
      ]
   ]
   
   ask sessions with [thread-count = 0] [ 
      set ttl ttl - 1
      if ttl = 0 [ die ]
   ]
...
to layout-threads
   ...
   __layout-circle threads with [idle?] max-pxcor - 1
end
...
to remove-tx-from-session
   set thread-count thread-count - 1
   if thread-count = 0 [ init-session ]
end
